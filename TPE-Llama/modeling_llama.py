...
class LlamaFlashAttention2Ours(LlamaAttention):
    ...
    def forward(
        ...
        assert token_ids.size()[1] == 2*q_len
        ...
        q_x, q_y = q_x.transpose(1, 2), q_y.transpose(1, 2) # [bsz, q_len, heads, head_dim]
        k_x, k_y = k_x.transpose(1, 2), k_y.transpose(1, 2) # [bsz, q_len, heads, head_dim]
        value_states = value_states.transpose(1, 2) # [bsz, q_len, heads, head_dim]

        v_x, v_y = value_states.clone(), value_states.clone() # [bsz, q_len, heads, head_dim]
        ...
        if q_len > 1:
            ...
            q_y, k_y, v_y = torch.gather(q_y, 1, token_col), torch.gather(k_y, 1, token_col), torch.gather(value_states, 1, token_col)
            inverse_indices = token_ids[..., q_len:].unsqueeze(2).unsqueeze(3).expand(-1, -1, self.num_heads, self.head_dim)

        padding_mask = padding_mask.long()
        dropout_rate = 0.0  # if not self.training else self.attn_dropout
        attn_output_x = self._flash_attention_forward(q_x, k_x, v_x, padding_mask, q_len, dropout=dropout_rate) # [bsz, q_len, heads, head_dim]
        attn_output_y = self._flash_attention_forward(q_y, k_y, v_y, padding_mask, q_len, dropout=dropout_rate) # [bsz, q_len, heads, head_dim]

        if q_len > 1:
            attn_output_y = torch.gather(attn_output_y, 1, inverse_indices) # [bsz, q_len, heads, head_dim]

        #router
        hidden_states_for_router = hidden_states.view(bsz, q_len, self.num_heads, self.head_dim)
        router_logits = self.gate_3(self.act_fn(self.gate_1(hidden_states_for_router)) * self.gate_2(hidden_states_for_router))
        routing_weights = nn.functional.softmax(router_logits, dim=-1, dtype=torch.float) # [bsz, q_len, heads, expert_nums]
        routing_weights = routing_weights.to(hidden_states.dtype)
        routing_weights = routing_weights.permute(3,0,1,2).contiguous() # [expert_nums,bsz, q_len, heads]

        attn_output = torch.stack([attn_output_x, attn_output_y], dim=0) # [expert_nums, bsz, q_len, heads, head_dim]
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        self.self_attn = (
            ...
            else LlamaFlashAttention2Ours(config=config)
        )
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(decoder_layer), hidden_states, input_ids, token_ids, pos_A, pos_B, attention_mask, position_ids
                )
            else:
